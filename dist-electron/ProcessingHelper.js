...
class ProcessingHelper {
        ...
        if (view === "queue") {
            ...
            if (lastPath.endsWith('.mp3') || lastPath.endsWith('.wav')) {
                ...
                try {
                    ...
                    const problemInfo = {
                        ...
                        input_format: { description: "Audio coding problem", parameters: [] },
            ...
            try {
                ...
                const problemInfo = {
                    ...
                    input_format: { description: "Coding problem from screenshot", parameters: [] },
                    output_format: { description: "Code solution", type: "code", subtype: detectedLanguage },
            ...
            try {
                ...
                const currentSolution = await this.llmHelper.solveCodingProblem(problemInfo.problem_statement, detectedLanguage);
                const currentCode = currentSolution.solution;
                // Debug the solution using vision model and image analysis
                const debugResult = await this.llmHelper.debugCode(currentCode, detectedLanguage, `Debug this solution using the provided images for additional context. Original problem: ${problemInfo.problem_statement}`);
                ...
                mainWindow.webContents.send(this.appState.PROCESSING_EVENTS.DEBUG_SUCCESS, { solution: finalDebuggedCode });
            }
            catch (error) {
    ...
    async processAudioBase64(data, mimeType, language = "auto") {
    ...
    async processAudioFile(filePath, language = "auto") {
        return this.llmHelper.analyzeAudioFile(filePath, language);
    }
    async processTextInput(text, language = "auto") {
    ...
    async processCodeFormatting(code, language = "auto") {
        // If language is auto, detect it first
        if (language === "auto") {
