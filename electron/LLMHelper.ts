...
import fs from "fs"
import { personalProfile, PersonalProfileHelper } from "./PersonalProfile"

export class LLMHelper {
  private model: GenerativeModel
  private questionCache: Map<string, { answer: string; timestamp: number; expiryTime: number }> = new Map()
  private readonly cacheExpirationMs = 30 * 60 * 1000; // 30 minutes cache
  private readonly codingPrompt = `You are an expert coding problem solver with mastery in:
...
ALWAYS provide complete, working code solutions with formatting that matches the example quality for the specific language.`
  ...
  private isPersonalQuestion(text: string): boolean {
    ...
    const projectNames = personalProfile.projects.map(project => 
      project.name.toLowerCase().replace(/[^\w\s]/g, ' ').trim()
    );
   ...
   */
  private generatePersonalResponse(question: string): string {
    const lowerQuestion = question.toLowerCase();
    
    // Self introduction
    if (lowerQuestion.includes('tell me about yourself') || lowerQuestion.includes('introduce yourself')) {
      return `${PersonalProfileHelper.getIntroduction()} ${PersonalProfileHelper.getTechnicalSkillsSummary()} I'm passionate about building innovative software solutions and continuously learning new technologies.`;
    }
    
    // Experience questions
    if (lowerQuestion.includes('experience') || lowerQuestion.includes('work')) {
      return `I have ${personalProfile.experience.totalYears}+ years of experience in full-stack development and AI/ML projects. I'm currently a B.Tech Computer Engineering student with hands-on experience in building innovative applications. ${PersonalProfileHelper.getProjectHighlight()}`;
    }
    
    // Skills questions
    if (lowerQuestion.includes('skills') || lowerQuestion.includes('technical')) {
      return `${PersonalProfileHelper.getTechnicalSkillsSummary()} I'm particularly strong in ${personalProfile.skills.frameworksLibraries.slice(0, 3).join(', ')} for development and ${personalProfile.skills.programmingLanguages.slice(0, 3).join(', ')} as programming languages. I also have experience with tools like ${personalProfile.skills.databasesTools.slice(0, 2).join(' and ')}.`;
    }
    
    // Specific project questions - check for mentions of actual project names
    for (const project of personalProfile.projects) {
    ...
    if (lowerQuestion.includes('project') || lowerQuestion.includes('built')) {
      ...
      return `${PersonalProfileHelper.getProjectHighlight()} This project showcases my ability to ${mainProject.highlights[0].toLowerCase()}, ${mainProject.highlights[1].toLowerCase()}. I've also worked on other projects including ${personalProfile.projects[1]?.name}, ${personalProfile.projects[2]?.name}, and ${personalProfile.projects[3]?.name || 'several other innovative applications'}.`;
    }
    
    // Strengths questions
    if (lowerQuestion.includes('strength') || lowerQuestion.includes('good at')) {
      return `${PersonalProfileHelper.getStrengthsAndGoals()} I'm particularly good at problem-solving complex technical challenges, learning new technologies quickly, and collaborating effectively with cross-functional teams. I also have a strong attention to detail and focus on writing clean, maintainable code.`;
    }
    
    // Career goals
    if (lowerQuestion.includes('career') || lowerQuestion.includes('future') || lowerQuestion.includes('goals')) {
      return `My career goal is to ${personalProfile.careerGoals[0].toLowerCase()}. I want to ${personalProfile.careerGoals[1].toLowerCase()} and ${personalProfile.careerGoals[2].toLowerCase()}. I'm particularly interested in working on innovative projects that have real-world impact and allow me to grow both technically and professionally.`;
    }
    
    // Why hire you
    if (lowerQuestion.includes('hire you') || lowerQuestion.includes('why choose')) {
      return `You should hire me because I bring a unique combination of strong technical skills, practical experience, and passion for innovation. With ${personalProfile.experience.totalYears}+ years of experience, I've proven my ability to deliver high-quality solutions using modern technologies. I'm a quick learner who adapts well to new challenges and contributes positively to team dynamics. Plus, I have hands-on experience with projects like ${personalProfile.projects[0].name}, which demonstrates my ability to work with cutting-edge technologies.`;
    }
    
    // Default personal response
    return `Thank you for the question. I'm ${personalProfile.name}, a ${personalProfile.title} with ${personalProfile.experience.totalYears}+ years of experience. I'm passionate about building innovative software solutions and have expertise in both frontend and backend development. I'm always eager to learn new technologies and contribute to meaningful projects.`;
  }

  /**
   * Helper method to detect if a question is interview-style
   */
  private isInterviewStyleQuestion(text: string): boolean {
   ...
   */
  public async solveCodingProblem(problemDescription: string, language: string) {
    try {
...
- Proper spacing and indentation`
   ...
   */
  public async optimizeCode(code: string, language: string = "python", requirements?: string) {
    try {
      const prompt = `${this.codingPrompt}
...
If you make significant changes, briefly explain the improvements.`
   ...
   */
  public async solveCodingProblemFromImage(imagePath: string, language?: string) {
    try {
...
- Format: Professional ${language} code formatting`
   ...
   */
  public async analyzeTextInput(text: string, language: string = "python") {
    try {
      const prompt = `${this.codingPrompt}
...
Focus on correctness, efficiency, and professional code formatting.`
   ...
   */
  public async analyzeImageFile(imagePath: string, language?: string) {
    try {
      const imageData = await fs.promises.readFile(imagePath)
      const imagePart = {
        inlineData: {
...
Return EXACTLY one of: CODING_PROBLEM, CODE_SCREENSHOT, TECHNICAL_DIAGRAM, UI_MOCKUP, ERROR_SCREENSHOT, GENERAL_QUESTION`
   ...
   */
  private async handleCodingProblemImage(imagePart: any, language?: string) {
    if (!language) {
...
Include a brief explanation of your approach if the logic is complex.`
   ...
   */
  private async handleCodeScreenshot(imagePart: any, language?: string) {
...
Provide a comprehensive but clear explanation that helps understand the code.`
   ...
   */
  private async handleTechnicalDiagram(imagePart: any) {
...
Provide insights that demonstrate understanding of the system design.`
   ...
   */
  private async handleUIMockup(imagePart: any) {
...
Provide practical advice for implementing this design.`
   ...
   */
  private async handleErrorScreenshot(imagePart: any, language?: string) {
...
Focus on practical solutions and clear explanations.`
   ...
   */
  private async handleGeneralImageQuestion(imagePart: any) {
...
Provide a thorough answer based on the image content.`
   ...
   */
  public async analyzeAudioFile(audioPath: string, language: string = "python") {
    try {
...
Focus on correctness, efficiency, and professional formatting.`
   ...
   */
  public async analyzeAudioFromBase64(data: string, mimeType: string, language: string = "auto") {
    try {
      const audioPart = {
        inlineData: {
          data,
...
Return only the transcribed text.`
   ...
   */
  public async debugCode(code: string, language: string = "python", errorDescription?: string) {
    try {
      const prompt = `${this.codingPrompt}
...
Explain what was fixed and improved.`
   ...
   */
  public async formatCode(code: string, language?: string) {
    try {
...
Focus on identifying the EXACT intended language or response type.`
   ...
   */
  public async answerQuestion(question: string, preDetectedLanguage?: string): Promise<{ answer: string; timestamp: number }> {
    try {
...
- Be specific about which technologies or languages you're discussing`;
   ...
   */
  private getCachedAnswer(question: string): { answer: string; timestamp: number } | null {
   ...
   */
  private cacheAnswer(question: string, answer: string, timestamp: number): void {
    const normalizedQuestion = question.toLowerCase().trim();
   ...
   * @param preDetectedLanguage - Optional pre-detected language to skip detection
   */
  public async answerInterviewQuestion(question: string, preDetectedLanguage?: string): Promise<{ answer: string; timestamp: number }> {
    try {
   ...
   */
  private categorizeInterviewQuestion(question: string, detectedLanguage: string): string {
    const lowerQuestion = question.toLowerCase();
  ...
  private generateInterviewPrompt(question: string, detectedLanguage: string, questionType: string): string {
    ...
    const relevantExperience = questionType !== 'general' ? 
      PersonalProfileHelper.getRelevantExperience(question) : [];
    ...
    const basePrompt = `You are ${personalProfile.name}, ${personalProfile.title} with ${personalProfile.experience.totalYears}+ years of experience. 
...
FORMAT EXAMPLE:
"[Technology/Concept], from my perspective as a ${personalProfile.title}, is [clear definition]. 
...
From my experience working on ${personalProfile.projects[0].name}, [personal connection and practical insights].
...
- Reference your experience from ${personalProfile.projects[0].name} and other projects
...
- Reference best practices you've learned in your ${personalProfile.experience.totalYears}+ years`,
...
- Be balanced and objective, mentioning specific use cases`,
      ...
      concept: `- Start with a crystal-clear definition as a ${personalProfile.title}
...
- Connect to your work with ${personalProfile.skills.programmingLanguages[0]} and ${personalProfile.skills.frameworksLibraries[0]}`,
      
      frontend: `- Follow the structured format focusing on ${personalProfile.skills.frameworksLibraries[0]}, JavaScript, React
...
- Use your ${personalProfile.experience.totalYears}+ years of experience for context
...
- Maintain professional interview tone as ${personalProfile.name}
