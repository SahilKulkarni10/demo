...
export class LLMHelper {
  ...
  private readonly systemPrompt = `You are an expert quiz solver with 100% accuracy. You excel at:
...
ACCURACY REQUIREMENT: Your answer must be 100% correct. Take time to verify thoroughly.`
  ...
  private enhanceMathPrompt(basePrompt: string): string {
    return `${basePrompt}
...
- Provide only the correct answer without showing work.`;
   ...
   */
  public async analyzeQuizOrTextInput(text: string) {
    try {
      const prompt = `${this.systemPrompt}
...
- Provide only the correct answer without showing work.`
   ...
   */
  public async solveMathProblem(problemText: string, options?: string[]) {
    try {
...
CRITICAL: Complete all mathematical work internally, verify calculations three times, then provide only the correct answer.`;
   ...
   */
  public async solveCodingProblem(problemText: string, language?: string) {
    try {
